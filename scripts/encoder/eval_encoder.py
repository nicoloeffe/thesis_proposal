...
def sequence_regime_probe(
    ...
    for ep in pure_eps:
        ...
        for start in range(0, len(ep_indices) - seq_len + 1, stride):
            ...
            # Keep only truly consecutive windows
    ...
    if len(starts) == 0:
        print(" WARNING: nessuna sequenza estratta")
        return {
            "error": "insufficient_sequences",
            "n_sequences": 0,
        }

    print(f" Total candidate sequences: {len(starts)}")
    print(f" Candidate regime distribution: {np.bincount(labels, minlength=len(REGIMES))}")

    # Balanced subsampling AFTER sequence extraction
    rng = np.random.default_rng(seed)
    n_classes = len(REGIMES)
    per_class = max_sequences // n_classes
    ...
    for c in range(n_classes):
        ...
        keep_c = idx_c[:min(len(idx_c), per_class)]
        rem_c = idx_c[min(len(idx_c), per_class):]

        keep.append(keep_c)
        remaining.append(rem_c)

    keep = np.concatenate(keep) if len(keep) else np.array([], dtype=np.int64)
    remaining = np.concatenate(remaining) if len(remaining) else np.array([], dtype=np.int64)

    if len(keep) < min(max_sequences, len(starts)):
        rng.shuffle(remaining)
        n_extra = min(max_sequences - len(keep), len(remaining))
        keep = np.concatenate([keep, remaining[:n_extra]])
    ...
    print(f" Regime distribution: {np.bincount(labels, minlength=len(REGIMES))}")
    if n_seq < 100:
...
if __name__ == "__main__":
    ...
    p.add_argument("--out_dir",   type=str,   default="validation/encoder")
